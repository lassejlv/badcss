...
export function ButtonComponent(tag) {
  ...
  let buttonSize = tag.getAttribute("size");
  const withRing = tag.getAttribute("withRing");
  const overwriteColor = tag.getAttribute("color");
  const cursorDisabled = tag.getAttribute("disabled:cursor");
  const isBold = tag.getAttribute("isBold");

  // Check if the button has a size
  if (!buttonSize) {
    Log({
      message: `No size found for button, defaulting to md`,
      type: "warn",
    });
    buttonSize = "md";
  }
  ...
  if (!allowedVariants.includes(buttonType) && !overwriteColor) {
  ...
  styles.innerHTML = `
        ...
        */
          ...
          .button-${buttonType}${withRing === "true" ? "-ring" : ""}:disabled {
            opacity: 0.5;
            ${cursorDisabled === "not-allowed" ? "cursor: not-allowed;" : ""}
          }
